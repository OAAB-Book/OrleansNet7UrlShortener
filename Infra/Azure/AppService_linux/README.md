...
```sh
az deployment sub create --name orleans_net7webapp_linux_demo01 --location [azure_datacenter_region_you_choose] --template-file ./main.bicep  --parameters deploy_region=[azure_datacenter_region_you_choose] ./parameters.json
```
...
```sh
az deployment sub delete --name orleans_net7webapp_linux_demo01
```

And also be sure to delete the resource group `orleans_net7webapp_linux_demo` either on Azure Portal or via cli command:
```sh
az group delete --name orleans_net7webapp_linux_demo
```
